...
class MainActivity : BaseActivityWithBanner<ActivityMainBinding>() {
    ...
    private fun showSdkVersionInfo() {
        val nkhVersion = BuildConfig.FIREANTS_ADS_VERSION
        val adsVersion = BuildConfig.PLAY_SERVICES_ADS_VERSION
        mBinding.tvSdkVersion.text = "NKH Studio v$nkhVersion • Ads SDK v$adsVersion"
    }
    ...
    override fun onClickViews() {
        ...
        mBinding.buttonHello.click {
            FireAntsTrackingHelper.logEventClick(this::class.java.simpleName, "buttonHello", null)
        }
        ...
        nativeDashboardLive.observe(this) { ad ->
            if (ad != null) {
                when (currentPreviewTarget) {
                    "small" -> showNativePreview(
                        ad,
                        mBinding.flNativeSmall,
                        mBinding.shimmerNativeSmall
                    )

                    "full" -> showNativePreview(
                        ad,
                        mBinding.flNativeFull,
                        mBinding.shimmerNativeFull
                    )
                }
                ...
                if (mBinding.shimmerCustomizationPreview.visibility == View.VISIBLE
                    ...
                    showNativePreview(
                        ad,
                        mBinding.flCustomizationPreview,
                        mBinding.shimmerCustomizationPreview
                    )
                }
            } else {
    ...
    private fun highlightOrderButton(selected: com.google.android.material.button.MaterialButton) {
        ...
        allButtons.forEach { btn ->
            if (btn == selected) {
                ...
                btn.strokeColor =
                    android.content.res.ColorStateList.valueOf(getColor(R.color.color_main))
            } else {
                btn.setTextColor(getColor(R.color.textPrimary))
                btn.strokeColor =
                    android.content.res.ColorStateList.valueOf(getColor(R.color.color_divider))
            }
        }
    }
    ...
    private val flags = listOf(
        FlagInfo("shouldDisplayInterOnboarding") {
            FireAntsAdSdk.getInstance()
                .shouldDisplayInterOnboarding
        },
        FlagInfo("shouldDisplayInterWelcomeBack") {
            FireAntsAdSdk.getInstance()
                .shouldDisplayInterWelcomeBack
        },
        FlagInfo("shouldDisplayNativeOnboardingNormal1") {
            FireAntsAdSdk.getInstance()
                .shouldDisplayNativeOnboardingNormal1
        },
        FlagInfo("shouldDisplayNativeOnboardingNormal2") {
            FireAntsAdSdk.getInstance()
                .shouldDisplayNativeOnboardingNormal2
        },
        FlagInfo("shouldDisplayNativeOnboardingFull1") {
            FireAntsAdSdk.getInstance()
                .shouldDisplayNativeOnboardingFull1
        },
        FlagInfo("shouldDisplayNativeWelcomeBack") {
            FireAntsAdSdk.getInstance()
                .shouldDisplayNativeWelcomeBack
        },
        FlagInfo("shouldDisplayWidgetUninstall") {
            FireAntsAdSdk.getInstance()
    ...
    private fun setResumeMode(mode: ResumeAdsEntryMode) {
        ...
        try {
            ...
            if (openResumeConfig != null) {
                updatedAds["open_resume"] =
                    openResumeConfig.copy(isEnable = (mode == ResumeAdsEntryMode.OPEN_RESUME))
            }
            if (nativeWelcomeConfig != null) {
                updatedAds["native_welcome"] =
                    nativeWelcomeConfig.copy(isEnable = (mode == ResumeAdsEntryMode.WELCOME))
            }
            if (interWelcomeConfig != null) {
                updatedAds["inter_welcome"] =
                    interWelcomeConfig.copy(isEnable = (mode == ResumeAdsEntryMode.WELCOME))
            }
    ...
    private fun updateResumeModeUI() {
        ...
        mBinding.tvResumeConfigsDetail.text =
            "open_resume: ${if (openResumeEnable) "ON" else "OFF"} • native_welcome: ${if (nativeWelcomeEnable) "ON" else "OFF"} • inter_welcome: ${if (interWelcomeEnable) "ON" else "OFF"}"

        // Highlight selected button
        val allButtons =
            listOf(mBinding.btnModeOpenResume, mBinding.btnModeWelcome, mBinding.btnModeNone)
