...
class AppLifecycleObserver : DefaultLifecycleObserver {
    ...
    override fun onStart(owner: LifecycleOwner) {
        ...
        if (currentActivity != null) {
            ...
            val shouldShowWelcome = !isDisable
                    && ResumeAdsEntryRule.shouldShowWelcomeOnResume()
                    && !AppOpenManager.getInstance().isInterstitialShowing
                    && !PremiumAccessManager.isPremium(currentActivity.applicationContext)
                    && FireAntsAdSdk.getInstance().shouldDisplayInterWelcomeBack
