package com.fireants.customactivitycrash;
...
import com.fireants.customactivitycrash.activity.FireAntsErrorActivity;
import com.fireants.customactivitycrash.config.FireAntsCrashConfig;
...
public final class CustomActivityOnCrash {
    ...
    private static final String EXTRA_CONFIG = "com.fireants.customactivitycrash.EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = "com.fireants.customactivitycrash.EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = "com.fireants.customactivitycrash.EXTRA_ACTIVITY_LOG";
    private static final String EXTRA_CUSTOM_CRASH_DATA = "com.fireants.customactivitycrash.EXTRA_CUSTOM_CRASH_DATA";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.fireants.customactivitycrash.ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.fireants.customactivitycrash.RESTART";
    private static final String CAOC_HANDLER_PACKAGE_NAME = "com.fireants.customactivitycrash";
    ...
    private static FireAntsCrashConfig config = new FireAntsCrashConfig();
                    ...
                    Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
                        if (config.isEnabled()) {
                            ...
                            if (hasCrashedInTheLastSeconds(application)) {
                                ...
                                boolean isConflictive = isStackTraceLikelyConflictive(throwable, errorActivityClass);
                                Log.d(TAG, "isStackTraceLikelyConflictive: " + isConflictive);
                                Log.d(TAG, "backgroundMode: " + config.getBackgroundMode() + " (SHOW_CUSTOM=" + FireAntsCrashConfig.BACKGROUND_MODE_SHOW_CUSTOM + ")");
                                Log.d(TAG, "isInBackground: " + isInBackground);
                                Log.d(TAG, "lastActivityCreatedTimestamp: " + lastActivityCreatedTimestamp);
                                ...
                                if (isConflictive) {
                                ...
                                } else if (config.getBackgroundMode() == FireAntsCrashConfig.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground
                                        || (lastActivityCreatedTimestamp >= new Date().getTime() - TIME_TO_CONSIDER_FOREGROUND_MS)) {
                                    Log.d(TAG, "Conditions met, will launch error activity");
                                    ...
                                    try {
                                        Log.d(TAG, "Attempting to launch error activity: " + errorActivityClass.getName());
                                        application.startActivity(intent);
                                        Log.d(TAG, "Error activity launched successfully");
                                    } catch (Throwable t) {
                                        Log.e(TAG, "Failed to launch error activity: " + errorActivityClass.getName(), t);
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
                                    }
                                } else if (config.getBackgroundMode() == FireAntsCrashConfig.BACKGROUND_MODE_CRASH) {
                                    if (oldHandler != null) {
                                        oldHandler.uncaughtException(thread, throwable);
                                        return;
                                    }
     ...
     */
    @Nullable
    public static FireAntsCrashConfig getConfigFromIntent(@NonNull Intent intent) {
        FireAntsCrashConfig config = (FireAntsCrashConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull FireAntsCrashConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull FireAntsCrashConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull FireAntsCrashConfig config) {
        if (config.getEventListener() != null) {
            try {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static FireAntsCrashConfig getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull FireAntsCrashConfig config) {
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.fireants.customactivitycrash.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.fireants.customactivitycrash.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.fireants.customactivitycrash.ERROR" />,
    ...
    private static Class<? extends Activity> guessErrorActivityClass(@NonNull Context context) {
        ...
        if (resolvedActivityClass == null) {
            resolvedActivityClass = FireAntsErrorActivity.class;
        }
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.fireants.customactivitycrash.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
